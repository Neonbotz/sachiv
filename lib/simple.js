  ...
  /**
    ...
    */
    ...
    conn.sendButtonDoc = async (jid, content, footerText, button1, id1, quoted, options) => {
      ...
      const buttonMessage = {
        document: { url: "https://wa.me/6287719424487" },
        mimetype: global.fdocs,
        fileName: wm,
...
END:VCARD`.trim()
    ...
    conn.reply = (jid, text = '', quoted, options) => {
        return Buffer.isBuffer(text) ? this.sendFile(jid, text, 'file', '', quoted, false, options) : conn.sendMessage(jid, { ...options, text, contextInfo: { mentionedJid: conn.parseMention(text), forwardingScore: 4644, isForwarded: true } }, { quoted, ...options })
    }
